...
import pickle  # Remember Me özelliği için
import os
...
"")
        ...
        # Minimize ve Close butonlarının işlevselliğini ekleyin
        self.minimize_button.clicked.connect(Login.showMinimized)  # Minimize işlevi
        self.close_button.clicked.connect(Login.close)  # Kapatma işlevi
        ...
        # Forgot Password tıklanınca mesaj gösterme
        self.forgot_button.clicked.connect(self.show_forgot_password_message)
        ...
        # Remember Me özelliği için kullanıcı adı ve şifreyi yükleyin
        self.load_credentials()

        # Remember Me işlevini kaydetme
        self.login_button.clicked.connect(self.save_credentials_if_remember)
    ...
    def save_credentials_if_remember(self):
        if self.checkBox.isChecked():
            credentials = {"username": self.lineEdit.text(), "password": self.lineEdit_2.text()}
    ...
    def load_credentials(self):
        if os.path.exists("credentials.pkl"):
            with open("credentials.pkl", "rb") as file:
                ...
                self.lineEdit.setText(credentials.get("username", ""))
                self.lineEdit_2.setText(credentials.get("password", ""))
                self.checkBox.setChecked(True)

    def show_forgot_password_message(self):
        QMessageBox.information(None, "Password Recovery", "To reset your password, please contact support.")
